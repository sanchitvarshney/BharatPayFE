...
import { getR18Data } from "@/features/report/report/reportSlice";
import { useSocketContext } from "@/components/context/SocketContext";
import R18ReportTable from "@/table/report/R18ReportTable";
dayjs.extend(customParseFormat);
const { MonthPicker } = DatePicker;
// const dateFormat = "MM";

const R22Report: React.FC = () => {
  const [colapse, setcolapse] = useState<boolean>(false);
  const [date, setDate] = useState<any | null>(null);
  const [pageSize, setPageSize] = useState(20);
  const { emitDownloadR22Report, isConnected } = useSocketContext();
  ...
  const handleDownload = () => {
    ...
    emitDownloadR22Report({ fromDate: `${date}-01`, toDate: `${date}-31` });
  };
  ...
  const handlePageSizeChange = (size: number) => {
    setPageSize(size);
    if (date) {
      dispatch(getR18Data({ ...date, page: 1, limit: size }));
    }
  };
      ...
      >
        ...
        <div className="flex flex-col p-[20px] gap-[20px] mt-[20px] overflow-hidden">
          <MonthPicker
            ...
            onChange={(value) => {
              setDate(value ? value.format("YYYY-MM") : null);
            }}
