...
const R8Report: React.FC = () => {
  ...
  const { r8ReportLoading, r8Report } = useAppSelector((state) => state.report);
  ...
  const getreport = () => {
    if (dateRange.from && dateRange.to) {
      dispatch(getr8Report({ from: dateRange.from.format("DD-MM-YYYY"), to: dateRange.to.format("DD-MM-YYYY") }));
    }
  };
        ...
        <div className={` transition-all h-full ${colapse ? "min-w-0 max-w-[0px]" : "min-w-[400px] max-w-[400px] "}  overflow-y-auto overflow-x-hidden border-r border-neutral-400/70   `}>
          ...
          <div className="p-[20px] ">
            ...
            <div className="mt-[20px] flex items-center justify-between">
              <LoadingButton onClick={getreport} loading={r8ReportLoading} loadingPosition="start" startIcon={<Icons.search fontSize="small" />} variant="contained">
                Search
              </LoadingButton>
              <LoadingButton
                disabled={!r8Report}
