...
import { getPartCodeReport } from "@/features/report/report/reportSlice";
...
const PartCodeConversionReport: React.FC = () => {
  ...
  const { isConnected, emitDownloadPartCodeConversionReport } =
    useSocketContext();
  const handleDownloadDetails = () => {
    ...
    if (isConnected) {
      emitDownloadPartCodeConversionReport(payload);
    } else {
                    ...
                    onClick={() => {
                      if (filterType === "DATE") {
                        dispatch(
                          getPartCodeReport({
                            type: filterType,
                            fromDate: dayjs(reportDate.from).format(
                              "YYYY-MM-DD",
                            ),
                            toDate: dayjs(reportDate.to).format("YYYY-MM-DD"),
                          }),
