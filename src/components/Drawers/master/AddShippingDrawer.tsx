...
const AddShippingDrawer: React.FC<Props> = ({ open, setOpen }) => {
  return (
    <div>
      <CustomDrawer open={open} onOpenChange={setOpen}>
        <CustomDrawerContent side="right" className="min-w-[50%] p-0">
          <CustomDrawerHeader className="h-[50px] p-0 flex flex-col justify-center px-[20px] space-y-0 bg-zinc-200 gap-0">
            <CustomDrawerTitle className="text-slate-600 font-[500] p-0">Add Shipping Address</CustomDrawerTitle>
          </CustomDrawerHeader>
          <div className="h-[calc(100vh-100px)] overflow-y-auto flex flex-col gap-[30px] px-[20px] ">
            ...
            <fieldset className="border p-[20px] rounded-md">
              ...
              <div className="grid grid-cols-2 gap-[30px]">
                <CustomInput label="Address label" required />

                <CustomInput label="Company Name" required />

                <CustomInput label="Pan No." required />
                <CustomInput label="GST No." required />
                <CustomInput label="CIN No" required />
                <CustomSelect placeholder={"State"} required />
              </div>
              <div className="flex flex-col mt-[30px]">
                <Label className="text-slate-500 text-[15px] font-[400]">
                  Enter Complete Address <span className="text-red-500">*</span>
                </Label>
...
export default AddShippingDrawer;
