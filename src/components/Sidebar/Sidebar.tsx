...
const SIDEBAR_INJECTED_STYLES = `
  ...
  .sub-menu-tree-prefix { display: inline-flex; flex-shrink: 0; align-self: stretch; }
  .sub-menu-tree-guide-continue, .sub-menu-tree-guide-empty, .sub-menu-tree-branch {
    width: ${SUB_MENU_TREE_STEP}px; position: relative; flex-shrink: 0; align-self: stretch;
  }
  .sub-menu-tree-guide-continue::before {
    content: ""; position: absolute; left: 8px; top: 0; bottom: 0; border-left: 1px solid #bdbdbd;
  }
  .sub-menu-tree-branch.is-mid::before {
    content: ""; position: absolute; left: 8px; top: 0; bottom: 0; border-left: 1px solid #bdbdbd;
  }
  .sub-menu-tree-branch.is-mid::after, .sub-menu-tree-branch.is-last::after {
    content: ""; position: absolute; left: 8px; top: 50%; width: 10px; border-top: 1px solid #bdbdbd;
  }
  .sub-menu-tree-branch.is-last::before {
    content: ""; position: absolute; left: 8px; top: 0; height: 50%; border-left: 1px solid #bdbdbd;
  }
  .sub-menu-tree-row { display: flex; align-items: stretch; min-height: 36px; }
  .sub-menu-tree-row-content { flex: 1; min-width: 0; }
  .main-sidebar-scroll { overflow-y: auto; overflow-x: hidden; }
  .main-sidebar-scroll::-webkit-scrollbar { width: 6px; }
  .main-sidebar-scroll::-webkit-scrollbar-thumb { background: #c1c1c1; border-radius: 3px; }
  .main-sidebar-scroll::-webkit-scrollbar-track { background: transparent; }
`;
    ...
    <Box sx={{ position: "relative" }}>
      ...
      <CircularProgress
        ...
        sx={(theme) => ({
          ...
          ...theme.applyStyles("dark", { color: "#308fe8" }),
        })}
        size={40}
        thickness={4}
        {...props}
...
function findActiveMenuItem(
  ...
  for (const item of items) {
    ...
    if (item.children) {
      const nextHeadingKeys = item.isHeading ? [...headingKeys, item.key] : headingKeys;
      const currentParentKey = !item.isHeading && item.children ? item.key : parentKey;
      const result: ActiveMenuResult | null = findActiveMenuItem(
        item.children,
        currentPath,
...
function filterItemsByIsShown(items: SidebarMenuItem[]): SidebarMenuItem[] {
  return items
    .map((item) => {
      ...
      if (item.children?.length) {
        ...
        if (filteredChildren.length === 0 && item.isShown !== true) return null;
...
const SidebarInner = ({
  ...
  const dashboardItem: SidebarMenuItem = useMemo(
    () => ({
      key: "dashboard",
      label: "Dashboard",
      icon: <MdHome />,
  ...
  const sidebar1Items = useMemo(() => [dashboardItem, ...items], [dashboardItem, items]);
  ...
  useEffect(() => {
    ...
    if (activeMenuItem) {
      if (activeMenuItem.parentKey) {
        ...
        if (!isSecondSidebarPin) setIsSecondSidebarCollapsed(true);
  ...
  const handleItemClick = useCallback(
    (key: string, hasChildren: boolean, item?: SidebarMenuItem, isInSubMenu = false) => {
      if (isInSubMenu) {
      ...
      if (hasChildren) {
        ...
        item.onClick();
        if (!isFirstSidebarPin) setShowSideBar(false);
        if (!isSecondSidebarPin) setIsSecondSidebarCollapsed(true);
      ...
      } else if (!isInSubMenu && !isSecondSidebarPin) {
        setActiveKey(null);
        setIsSecondSidebarOpen(false);
      }
    },
  ...
  const hoveredItem = useMemo(
    () => filteredSidebar1Items.find((item) => item.key === activeKey),
    [activeKey, filteredSidebar1Items],
  );

  const menuTierFontSize = (nestLevel: number) => {
  ...
  const subMenuChevronImg = (px: number) => (
    <img
  ...
  const renderSubMenuTreePrefix = (ancestorHasMore: boolean[], isLast: boolean) => (
    <span className="sub-menu-tree-prefix" aria-hidden="true">
      {ancestorHasMore.map((continues, index) => (
        <span
          key={`tree-guide-${index}`}
          className={continues ? "sub-menu-tree-guide-continue" : "sub-menu-tree-guide-empty"}
        />
      ))}
      <span className={`sub-menu-tree-branch ${isLast ? "is-last" : "is-mid"}`} />
    </span>
  );
  ...
  const renderList = (
    ...
    const headingPlayArrowSize = isSubMenu && nestLevel === 0 ? 18 : isSubMenu ? 16 : 20;
    const leafIconBoxSize = !isSubMenu ? 18 : nestLevel <= 1 ? 18 : nestLevel === 2 ? 16 : 14;
    ...
    return (
      <ul style={{ listStyle: "none", margin: 0, padding: 0,  }}>
        {arr.map((c, index) => {
          ...
          const continuesBelow = hasChildren && (isHeading ? isHeadingExpanded : false);
          const branchIsLast = isLast && !continuesBelow;
          const childAncestorHasMore = [...ancestorHasMore, !isLast || continuesBelow];
            ...
            return (
              <li key={reactKey}>
                <div className={showSubMenuTree ? "sub-menu-tree-row" : undefined}>
                  {showSubMenuTree && renderSubMenuTreePrefix(ancestorHasMore, branchIsLast)}
                  <div
                    className={showSubMenuTree ? "sub-menu-tree-row-content" : undefined}
                    onClick={() => handleItemClick(c.key, hasChildren, c, isSubMenu)}
                    style={{
                      padding: showSubMenuTree ? "10px 12px 10px 4px" : "16px 16px 8px 16px",
                      color: "#474545",
                    ...
                    onMouseEnter={(e) => {
                      if (hasChildren) e.currentTarget.style.backgroundColor = "#f0f0f0";
                    }}
                    onMouseLeave={(e) => {
                      if (hasChildren) e.currentTarget.style.backgroundColor = "transparent";
                    }}
                  >
                    {isSubMenu ? (
                      <span
                      ...
                      >
                        {nestLevel === 0 ? (
                          ...
                          subMenuChevronImg(
                            nestLevel === 1 ? SUB_MENU_CHILD_CHEVRON_SIZE : SUB_MENU_DEEP_CHEVRON_SIZE,
                          )
                        )}
                    ...
                    {hasChildren && (
                      <PlayArrowIcon
                        style={{
                          ...
                          flexShrink: 0,
                          transform: isHeadingExpanded ? "rotate(90deg)" : "rotate(0deg)",
                ...
                {hasChildren && isHeadingExpanded && (
                  <div style={{ padding: 0 }}>
                    {renderList(c.children!, alwaysShowText, isSubMenu, nestLevel + 1, childAncestorHasMore)}
                  </div>
                )}
              </li>
            );
            ...
            <li key={reactKey}>
              <div className={showSubMenuTree ? "sub-menu-tree-row" : undefined}>
                ...
                <div
                  ...
                  onMouseEnter={(e) => {
                    if (!isActive && !isPathActive) {
                      e.currentTarget.style.backgroundColor = isSubMenu ? "#e8f4fd" : "#d4edda";
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isActive && !isPathActive) {
                  ...
                  style={{
                    padding: showSubMenuTree ? "8px 12px 8px 4px" : shouldShowText ? "8px 16px" : "8px 12px",
                    ...
                    backgroundColor:
                      isActive || isPathActive
                        ...
                        : "transparent",
                    borderLeft:
                      isActive || isPathActive ? "3px solid #0d9489" : "3px solid transparent",
                ...
                >
                  ...
                  >
                    {typeof c.label === "string" ? <span>{c.label}</span> : c.label}
                  </div>
                </div>
              </div>
            </li>
  ...
  const subSidebarOpen = useMemo(
    () => Boolean(hoveredItem?.children && isSecondSidebarOpen),
    [hoveredItem, isSecondSidebarOpen],
  );
  const secondSidebarWidth = isSecondSidebarCollapsed ? 56 : 280;

  const rootWidth = useMemo(() => {
    if (showSideBar && subSidebarOpen) return 280 + secondSidebarWidth;
    if (showSideBar && !subSidebarOpen) return 280;
    if (!showSideBar && subSidebarOpen) return 56 + secondSidebarWidth;
    return 56;
  ...
  return (
    <>
      ...
      <div
        className="flex fixed left-0 z-[70] transition-[width] duration-300 ease-in-out"
        style={{
          top: topOffset,
          left: 0,
          width: rootWidth,
      ...
      >
        <div
          className={`h-full flex flex-col overflow-hidden ${
            showSideBar ? "w-[280px] shadow-[2px_0_8px_rgba(0,0,0,0.1)]" : "w-[56px] shadow-none"
          } transition-all duration-300 ease-in-out bg-[#f8f9fa] relative z-[99] flex-none border-r border-[#e0e0e0]`}
        >
          <div
            style={{
              color: "#333",
          ...
          >
            <img
              src={showSideBar ? "/bharatpay.svg" : "/bharatpay.svg"}
              alt="BharatPay"
              style={{
                width: showSideBar ? 40 : 32,
                height: showSideBar ? 40 : 32,
                objectFit: "contain",
                borderRadius: showSideBar ? 0 : "50%",
              }}
            />
            {showSideBar && (
          ...
          {menuLoading ? (
            ...
            <>
              <div style={{ padding: "8px 0", flexShrink: 0 }}>
                {renderList([dashboardItem])}
              </div>

              <div
                className="main-sidebar-scroll flex-1 min-h-0"
                style={{ padding: "0 0 8px 0" }}
              >
                {filteredApiItems.length > 0 && renderList(filteredApiItems)}
              </div>
            </>
          )}
...
<div style={{ flexShrink: 0, height: 56 }} />
          {showSideBar && (
            <button
              ...
              onMouseLeave={(e) => {
                ...
                e.currentTarget.style.backgroundColor = isFirstSidebarPin ? "orange" : "transparent";
              }}
              title={
                isFirstSidebarPin
        ...
        {subSidebarOpen && hoveredItem?.children && (
          <div
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  ...
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = isSecondSidebarPin ? "orange" : "transparent";
                  }}
                  title={
                    isSecondSidebarCollapsed
                      ? "Expand submenu"
            ...
            <div
              className={`sub-sidebar-scroll ${isSecondSidebarCollapsed ? "hide-scrollbar" : ""}`}
              style={{
